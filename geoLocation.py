...
# GUI Setup
root = tk.Tk()
root.title("မွေးဖွားရာမြို့ ရွေးချယ်ခြင်း")

# City Input
ttk.Label(root, text="မွေးဖွားရာမြို့:").grid(row=0, column=0, padx=5, pady=5, sticky="e")
city_entry = ttk.Entry(root, width=30)
city_entry.grid(row=0, column=1, padx=5, pady=5)

fetch_btn = ttk.Button(root, text="Online မှ ရယူရန်", command=fetch_coordinates)
fetch_btn.grid(row=0, column=2, padx=5, pady=5)

# Latitude
ttk.Label(root, text="Latitude:").grid(row=1, column=0, padx=5, pady=5, sticky="e")
lat_value = tk.StringVar()
lat_entry = ttk.Entry(root, textvariable=lat_value, width=20)
lat_entry.grid(row=1, column=1, padx=5, pady=5)

# Longitude
ttk.Label(root, text="Longitude:").grid(row=2, column=0, padx=5, pady=5, sticky="e")
lon_value = tk.StringVar()
lon_entry = ttk.Entry(root, textvariable=lon_value, width=20)
lon_entry.grid(row=2, column=1, padx=5, pady=5)

# Timezone
ttk.Label(root, text="Timezone:").grid(row=3, column=0, padx=5, pady=5, sticky="e")
timezone_value = tk.StringVar(value="Auto")
timezone_entry = ttk.Entry(root, textvariable=timezone_value, width=20)
